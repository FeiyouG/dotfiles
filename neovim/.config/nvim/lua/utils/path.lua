...
function M.expand(...)
  local joined_path = M.join(...)
  local paths = M.glob(joined_path)
  if #paths == 0 then
    local path = vim.fn.expand(joined_path)
...
-- MARK: System
M.XDG_DIR = {
  CONFIG = M.glob_with_default("$HOME/.config", "$XDG_CONFIG_HOME"):first(""),
  CACHE = M.glob_with_default("$HOME/.cache", "$XDG_CACHE_HOME"):first(""),
  DATA = M.glob_with_default("$HOME/.local/share", "$XDG_DATA_HOME"):first(""),
  STATE = M.glob_with_default("$HOME/.local/state", "$XDG_STATE_HOME"):first(""),
}

-- MARK: Plugin related Paths
M.packer = {
  install_path = M.expand(vim.fn.stdpath("data"), "site/pack/packer/start/packer.nvim"):first(""),
  snapshot_path = M.expand(vim.fn.stdpath("cache"), "packer.nvim"):first(""),
}

M.mason = {
  package_dir = M.expand(vim.fn.stdpath("data"), "mason/packages"):first(""),
  bin_dir = M.expand(vim.fn.stdpath("data"), "mason/bin"):first(""),
}
...
-- MARK: Python
M.python = {
  debugby_exec = M.glob(M.mason.install_dir, "packages/debugpy/venv/bin/python").first(""),
  venvPath = M.expand(M.XDG_DIR.DATA, "virtualenvs").first(""),
}

M.xml = {
  lemminx_cache = M.expand(M.XDG_DIR.CACHE, "lemminx").first(""),
}
